...
export class CumulocityView implements vscode.TreeDataProvider<EPLApplication> {
  ...
  registerCommands(): void {
    if (this.context !== undefined) {
      this.context.subscriptions.push.apply(this.context.subscriptions, [
        ...
        vscode.commands.registerCommand(
          ...
          async (uri) => {
            try {
              ...
              let errorMessage =
                "Failed to register EPL App command, unknown error";
              if (error instanceof Error) {
                errorMessage = error.message;
              }
              vscode.window.showErrorMessage(
                "Error uploading " + uri.path + ":\n" + errorMessage,
              );
            }
          },
        ),
        ...
        //
        vscode.commands.registerCommand(
          "extension.c8y.openEplApp",
          async (element) => {
            const setting: vscode.Uri = vscode.Uri.parse(
              "untitled:" + element.label + ".mon",
            );
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
    _item?: EPLApplication | undefined,
  ): Promise<undefined | EPLApplication[]> {
    //await this.refresh();
